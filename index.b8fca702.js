...
function updateBoard() {
    for(let row = 0; row < rows.length; row++)for(let cell = 0; cell < rows[row].cells.length; cell++){
        const currentCell = rows[row].cells[cell];
        currentCell.textContent = game.board[row][cell] || "";
        for(let i = 0; i < currentCell.classList.length; i++)if (currentCell.classList[i].startsWith("field-cell--")) {
...
function handleKeyDown(e) {
    ...
    else if (game.getStatus() === "win") messageWin.classList.remove("hidden");
}
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    moveLeft() {
        ...
        this.board = this.board.map((row)=>{
            ...
            const newRow = [];
            for(let i = 0; i < valuesNotZero.length; i++)if (valuesNotZero[i] === valuesNotZero[i + 1]) {
                const mergedCells = valuesNotZero[i] * 2;
                newRow.push(mergedCells);
                this.score += mergedCells;
                this.checkWin(mergedCells);
                i++;
            } else newRow.push(valuesNotZero[i]);
    ...
    moveUp() {
        ...
        for(let i = 0; i < boardSize; i++)newBoard[i] = new Array(boardSize).fill(0);
        for(let col = 0; col < boardSize; col++){
            ...
            const newCol = [];
            for(let i = 0; i < column.length; i++)if (column[i] === column[i + 1]) {
                const mergedCells = column[i] * 2;
                newCol.push(mergedCells);
                this.score += mergedCells;
                this.checkWin(mergedCells);
                i++;
            } else newCol.push(column[i]);
            while(newCol.length < boardSize)newCol.push(0);
            for(let row = 0; row < boardSize; row++)newBoard[row][col] = newCol[row];
        }
        this.board = newBoard;
        if (JSON.stringify(this.board) !== prevBoard || this.isEmptyBoard()) {
            this.addRandomCell();
    ...
    moveDown() {
        ...
        for(let col = 0; col < boardSize; col++){
            ...
            const newCol = [];
            for(let i = 0; i < reversedCol.length; i++)if (reversedCol[i] === reversedCol[i + 1]) {
                const mergedCells = reversedCol[i] * 2;
                newCol.push(mergedCells);
                this.score += mergedCells;
                this.checkWin(mergedCells);
                i++;
            } else newCol.push(reversedCol[i]);
            while(newCol.length < boardSize)newCol.push(0);
            newCol.reverse();
            for(let row = 0; row < boardSize; row++)newBoard[row][col] = newCol[row];
        }
   ...
   */ restart() {
    ...
    addRandomCell() {
        ...
        const emptyCells = [];
        for(let row = 0; row < boardSize; row++){
            for(let col = 0; col < boardSize; col++)if (this.board[row][col] === 0) emptyCells.push({
                row,
                col
            });
        }
    ...
    checkLose() {
        ...
        if (canMove.every((move)=>move === false)) this.status = "lose";
    }
    checkWin(cell) {
        if (cell === 2048) this.status = "win";
    }
}
module.exports = Game;
